...
# Remove ancient site specifif code.
...
# Someday the login could be more complex, i.e. we start using a real session.  twl: apr 22 2008 Add
# alternatives as a workaround for broken OSX Leopard.  Do not use database (Vela) logging here since this sub
# is called from the Vela logging facility. If this is broken, Vela won't be able to write log messages. (Vela
# is the VEry LArge job queing system that keeps the job queue in a database.)
...
# Save data from the browser in some agnostic form prior to validating it and saving the data into the real
# tables. The server doesn't trust the browser.

# New: Save %ch (the reference $chr) as form data. Over write what ever is in the form data. We assume that
# %ch has been updated to include everything we want to save, and nothing we don't want. This is called at the
# end of a user script to save state. Make sure we are logged in. The low level code doesn't check login
# status.

# Old:Only call save_session() from a 2 script (i.e. insert_gene2.pl) where we are collecting data, but things
# are not right. If we can't write the data into one of the real tables, call save_session() to save all the
# hash values.  It also deletes session data for non-existant hash keys (presumably from some other web page.)

# insert new records

# update existing records

# delete non-existant records (which might be things like checkboxes which are now null) Unfortunately, delete
# may not apply to all circumstances, and then we'll have to modify this sub. That has already happened once
# with 'login'.
...
# ExecCGI.
#
# (Vela jobs are database resident on a db server separate from the cluster nodes running the jobs. The easy
# way to prevent cluster node logging collisions is to use a relational database where the transactions are
